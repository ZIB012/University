...
// We are studying caching in depth. Caching resources can be main memory or disk spaces. When you invoke cache,
    // by default the RDDs are cached in main memory, but there are overloads of these method where you can customize
    // where to cache the result

// Caching works until there are resources available, when there are no more resorces in your workers, some RDDs
    // need to be pruned from caching and that is done with the least recently used policy.
    // Since continuously caching can occupy a lot of resources, it's a good practice to unpersist RDDs when they are no more useful
