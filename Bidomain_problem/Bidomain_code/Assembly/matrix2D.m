...
switch Data.fem
    case 'P1'
%         for i = 2:last-1
%             m(i) = M(i,i-1) + M(i,i) + M(i,i+1);
%         end
%         m(1) = M(1,1) + M(1,2);
%         m(last) = M(last,last-1) + M(last,last);
%         M_L = diag(m);
        for j = 0:last/3-3
            x = 3*j + 1;
            m(x) = M(x,x) + M(x,x+1) + M(x,x+2);
            x = x + 1;
            m(x) = M(x,x-1) + M(x,x) + M(x,x+1);
            x = x + 1;
            m(x) = M(x,x-2) + M(x,x-1) + M(x,x);
        end
        M_L = diag(m);
    case 'P2'
        sum = 0;
...
%=============================
Matrices=struct('Ai',Vi - transpose(Ii) + teta*Ii + Si,'Ae',Ve - transpose(Ie) + teta*Ie + Se, 'f_i',f_i, 'f_e',f_e,  'S',S, 'M', M, 'M_L', M_L);
